...
class Device(db.Model, SerializerMixin):
    ...
    # Stores the Bluetooth device information for attendants.
    # Each attendant can have one registered BLE device used during attendance sessions.
    ...
    is_active = db.Column(db.Boolean, default=False)  # Active when attendant starts a session
    last_activated = db.Column(db.DateTime, nullable=True)

    # Relationship: A device belongs to one attendant
    attendant = db.relationship("User", back_populates="device")
